...
import { Baby, ChevronDown, HandHeart, HeartHandshake, Plus, Trash2, User, Users, type LucideIcon } from "lucide-react";
...
const BESKAEFTIGELSE_KORT: Record<Beskaeftigelse, string> = {
  loenmodtager: "Lønmodtager",
...
const KONSTELLATION_BESKRIVELSE: Record<string, string> = {
  "mor-far": "24 uger hver",
  "mor-medmor": "24 uger hver",
  "to-foraeldre": "24 uger hver",
  solo: "46 uger",
  "adoption-par": "24 uger hver",
  "adoption-solo": "46 uger",
};

const KONSTELLATION_IKON: Record<string, LucideIcon> = {
  "mor-far": Users,
  "mor-medmor": Users,
  "to-foraeldre": HeartHandshake,
  solo: User,
  "adoption-par": HandHeart,
  "adoption-solo": Baby,
};

const summaryKlasse =
  "flex cursor-pointer list-none items-center gap-3 rounded-lg px-4 py-3 hover:bg-gray-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 dark:hover:bg-gray-800 [&::-webkit-details-marker]:hidden";

const panelKlasse = "group min-w-0 rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/40";

/** One-line summary shown on a closed "Løn og detaljer" panel. */
function resume(f: Foraelder): string {
  const dele: string[] = [BESKAEFTIGELSE_KORT[f.beskaeftigelse]];
  if (f.beskaeftigelse === "loenmodtager" || f.beskaeftigelse === "selvstaendig") {
    dele.push(harEksempelLoen(f) ? `eksempelløn ${kr(f.maanedsloen)}` : `${kr(f.maanedsloen)} om måneden`);
  }
  if (f.loenUnderBarsel.length > 0) dele.push("løn under barsel");
  if (f.udskudteUger > 0) dele.push(`${f.udskudteUger} udskudte uger`);
  return dele.join(" · ");
}

/** Open a "Løn og detaljer" panel and move focus to it (used by links elsewhere on the page). */
export function aabenDetaljer(id: string) {
  const el = document.getElementById(id);
  if (!(el instanceof HTMLDetailsElement)) return;
  el.open = true;
  const roligt = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  el.scrollIntoView({ block: "center", behavior: roligt ? "auto" : "smooth" });
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div>
          ...
          >
            <option value="termin">{adoption ? "Forventet modtagelse" : "Barnet er ikke født endnu"}</option>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => opdater((p) => ({ ...p, antalBoern: Number(e.target.value) as BarselsPlan["antalBoern"] }))}
          >
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-2">
          ...
          {plan.konstellation === "solo" && (
            <label className="inline-flex items-center gap-2 text-sm text-gray-800 dark:text-gray-100">
              ...
              />
              Giv uger til en nærtstående (forælder eller søskende over 18 år)
            </label>
          )}
        </div>
      )}
      ...
      <div>
        <h3 className="text-base font-semibold text-gray-900 dark:text-white">Løn og detaljer</h3>
        <p className="mb-3 text-sm text-gray-600 dark:text-gray-300">Valgfrit. Bruges til at regne økonomien ud og bliver i din browser.</p>
        <div className={`grid items-start gap-3 ${plan.foraeldre.length > 1 ? "lg:grid-cols-2" : ""}`}>
          {plan.foraeldre.map((f, i) => (
            <ForaelderKort key={f.id} f={f} index={i} rolle={rolleFor(plan.konstellation, i)} opdater={(fn) => opdaterForaelder(i, fn)} />
          ))}
          <details id="barsel-detaljer-faelles" className={`${panelKlasse} ${plan.foraeldre.length > 1 ? "lg:col-span-2" : ""}`}>
            <summary className={summaryKlasse}>
              <span className="min-w-0 flex-1">
                <span className="block font-medium text-gray-900 dark:text-white">Indlæggelse og skat</span>
                <span className="block truncate text-sm text-gray-600 dark:text-gray-300">
                  {plan.indlaeggelsesUger > 0 ? `Barnet indlagt ${plan.indlaeggelsesUger} uger · ` : ""}Kommuneskat{" "}
                  {plan.kommuneskatPct.toLocaleString("da-DK")} %
                </span>
              </span>
              <ChevronDown className="h-5 w-5 shrink-0 text-gray-500 transition-transform group-open:rotate-180" aria-hidden="true" />
            </summary>
            <div className="grid gap-4 border-t border-gray-200 p-4 dark:border-gray-700 sm:grid-cols-2 lg:max-w-2xl">
              <TalFelt
                ...
                hjaelp="Fx hvis barnet er født for tidligt. Forlænger orloven."
              />
              <TalFelt
                label="Kommuneskat"
                ...
                hjaelp="Gennemsnittet i 2026 er ca. 25 %. Kirkeskat er ikke medregnet."
              />
            </div>
          </details>
        </div>
...
function ForaelderKort({
  ...
  return (
    <details id={`barsel-detaljer-${f.id}`} className={panelKlasse}>
      <summary className={summaryKlasse}>
        <span className={`h-3 w-3 shrink-0 rounded-full ${PERSON_DOT[index]}`} aria-hidden="true" />
        <span className="min-w-0 flex-1">
          <span className="block font-medium text-gray-900 dark:text-white">
            {f.navn || (rolle === "naertstaaende" ? "Nærtstående" : `Person ${index + 1}`)}
          </span>
          <span className="block truncate text-sm text-gray-600 dark:text-gray-300">{resume(f)}</span>
        </span>
        <ChevronDown className="h-5 w-5 shrink-0 text-gray-500 transition-transform group-open:rotate-180" aria-hidden="true" />
      </summary>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {(f.beskaeftigelse === "loenmodtager" || f.beskaeftigelse === "selvstaendig") && (
            <TalFelt
              ...
              hjaelp={
                harEksempelLoen(f)
                  ? "Eksempel. Skriv din egen løn for at se jeres økonomi."
                  : erLoenmodtager
                    ? undefined
                    : "Fuld sats kræver ca. 22.035 kr. om måneden (264.420 kr. om året)."
              }
            />
          )}
          {erLoenmodtager && (
        ...
        {erLoenmodtager && (
          <div className="mt-4">
            <div className="flex items-center gap-1">
              ...
              <InfoTip begreb="dagpenge" className="mb-1" />
            </div>
            <p className="mb-2 text-xs text-gray-600 dark:text-gray-400">
              Se din overenskomst eller kontrakt. Ugerne tæller fra din første orlovsuge, fx 14 uger med 100 % løn.
            </p>
